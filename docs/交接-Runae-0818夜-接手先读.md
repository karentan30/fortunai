...
## 网站必修（审计指出）
① ~~首页信任感~~✅已加真实信号 ② ~~定价CTA断链~~✅已修(→cross-check.html) ③ 🔴**仍缺：$99旗舰后端无聚合器**(未落地承诺·要么建"一次生成多体系汇总"聚合器·要么先别卖$99·`cross-check.html`是多体系页但确认它是否真串起后端各引擎出一份合报告)。

## 🔴 线上路由现状（新会话必看·易踩）
线上 `/` **仍是 `en.html`(英文Four Pillars·干净)**·中文玉版首页 `home-runae.html` **还没接到根路径**——部署后要访问得走 `runae.app/home-runae.html`。**是否把 `/` 换成中文玉版=待 Karen 定**(打第一波留学生/小红书用中文·但英文站有SEO)。换法=Caddy 或首页文件路由·别直接删 en.html。
...
## 下一步（顺序）
1. **部署**(命令见红线)→ **smoke-test 紫微/大六壬/西占/六爻/奇门真出报告**(字段路径未验)→ 通过才算 ready
2. Karen 定：①首页用哪块玉 ②`/` 是否换成中文玉版首页
3. $99旗舰聚合器(要么建·要么先下架$99档)
4. 报告套 persona/主题背景到"小红书档"(每报告一款·**用现成或经同意再生**)
5. 🔴 **推广=第一大缺口(0用户)**：@runae建号 + 小红书10条(`docs/小红书内容-第一批.md`·回国vs留外打头) + 留学生社区(Reddit r/chinesestudents/小红书留学区)
