...
import { insertPromptSectionBeforeHeading } from '../prompt/guidance.js';
 ...
 */
function buildMovementSteps(annualMovement) {
...
function buildQiSteps(sitianName) {
...
export function buildWuyunLiuqiPrompt(result, question, schools) {
    ...
    lines.push('', '【盘面资料】', `年干支：${result.input.yearGanZhi}${result.input.year === undefined ? '' : `（公历 ${result.input.year} 年）`}`, `岁运：${result.annualMovement.name}（${result.annualMovement.toneName}），${result.annualMovement.strength}（${result.annualMovement.yinYang}干）`, `司天：${result.sitian.name}`, `在泉：${result.zaiquan.name}`, `司天与中运：${result.annualRelation.kind}；${result.annualRelation.basis}`, `年度符会：${result.annualConformities.names.length ? result.annualConformities.names.join('、') : '未形成天符、岁会、太乙天符、同天符或同岁会'}`, '五步主客运：', ...result.movementSteps.map((step) => `${step.order}. ${step.label}（${step.periodRule}）：主运${step.hostMovement.toneName}（${step.hostMovement.element}）；客运${step.guestMovement.toneName}（${step.guestMovement.element}）${step.guestRole ? `（${step.guestRole}）` : ''}；主客关系${step.hostGuestRelation.kind}`), '六步主客气：', ...result.qiSteps.map((step) => `${step.order}. ${step.label}（${step.solarTerms.join('、')}）：主气${step.hostQi.name}；客气${step.guestQi.name}${step.guestRole ? `（${step.guestRole}）` : ''}；主客关系${step.hostGuestRelation.kind}`), '', '【传统依据】', '以年干定岁运太过不及，以年支定司天在泉，再看五步主客运与六步主客气的阶段关系。');
    return insertPromptSectionBeforeHeading(lines.join('\n'), '【问题】', buildPromptSchoolSection('wuyun-liuqi', schools));
}
