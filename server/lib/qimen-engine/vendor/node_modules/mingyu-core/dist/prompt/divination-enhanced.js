...
import { LIUCHONG_MAP, SIXTY_CYCLE } from '../ganzhi/index.js';
...
function formatLiurenInfo(data) {
    ...
    const shenShaText = (data.shenShaFacts?.length
...
function formatTarotInfo(data) {
    const cardLines = data.cards.map((card) => `- ${card.position}：${card.name}${card.reversed ? '（逆位）' : '（正位）'}${card.keywords.length ? `；关键词：${card.keywords.join('、')}` : ''}`);
    return [
        '占法：塔罗',
...
function formatAlmanacInfo(data) {
    ...
    const candidateByDate = new Map(evidenceAnalysis.candidates.map((candidate) => [candidate.date, candidate]));
    const preferredDays = data.days
        .filter((day) => !candidateByDate.get(day.date)?.status.includes('慎用'))
        .slice(0, 2);
    const cautiousDay = data.days.find((day) => candidateByDate.get(day.date)?.status.includes('慎用'));
    const topDays = [...preferredDays, ...(cautiousDay ? [cautiousDay] : [])].filter((day, index, items) => items.findIndex((item) => item.date === day.date) === index);
...
function formatLenormandInfo(data) {
    const cardLines = data.cards.map((card) => `- ${card.position}：${card.name}；关键词：${card.keywords.join('、')}`);
...
export function formatAstrolabeInfo(data) {
    ...
    const planetLines = data.planets
        ...
        .map((item) => `${item.label}${item.formatted}，第${item.house}宫${item.retrograde ? '，逆行' : ''}`);
    const aspectLines = data.aspects
        .slice(0, 5)
        .map((item) => `${item.body1}${item.symbol}${item.body2}（${item.type}，容许度${item.orb.toFixed(2)}°）`);
    return [
        '占法：星盘',
