...
export function buildHuangjiJingshiPrompt(result, question, schools) {
    ...
    lines.push('', '【周期资料】', `纪元年坐标：${input.epochYear}（某一元第一年）`, `目标年坐标：${input.year}`, `距纪元已过：${input.elapsedYears} 年`, `元：自纪元起第 ${position.yuan.indexFromEpoch} 元，${position.yuan.startYear} 至 ${position.yuan.endYear}`, `会：本元第 ${position.hui.indexInYuan} 会，${position.hui.startYear} 至 ${position.hui.endYear}`, `运：本元第 ${position.yun.indexInYuan} 运、本会第 ${position.yun.indexInHui} 运，${position.yun.startYear} 至 ${position.yun.endYear}`, `世：本元第 ${position.shi.indexInYuan} 世、本运第 ${position.shi.indexInYun} 世，${position.shi.startYear} 至 ${position.shi.endYear}`, `年：本世第 ${position.year.indexInShi} 年、本元第 ${position.year.indexInYuan} 年`, `周期边界：本世当前为第 ${result.progress.shi.currentYearIndex} 年，尚余 ${result.progress.shi.remainingYearsAfterCurrent} 个完整年；下一世始于 ${result.progress.shi.nextCycleStartYear}，下一运始于 ${result.progress.yun.nextCycleStartYear}`, '', '【传统依据】', '按一元十二会、一会三十运、一运十二世、一世三十年的元会运世层级定位。');
