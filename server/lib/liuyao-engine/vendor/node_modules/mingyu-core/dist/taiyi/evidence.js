...
const LIMITATION_FACT_LIMITATION = '限制事实用于约束太乙年计、七十二局、主客定算和十六神可以支持的解释范围，不得被反向当作现实结果或概率证据';
...
function buildLimitationFacts(calculationSteps, positionFacts, forceFacts, sixteenGodFacts) {
    const allFactKeys = [
    ...
    const definitions = [
        {
            ...
            promptText: '年计只适用于年度时间尺度；月、日、时计尚未完成节气时刻、章月、月法、日法、气应与小余的古籍历法链校勘，当前不得据年计结果替代推断',
...
export function buildTaiyiEvidence(data) {
    ...
    const limitationFacts = buildLimitationFacts(calculationSteps, positionFacts, forceFacts, sixteenGodFacts);
