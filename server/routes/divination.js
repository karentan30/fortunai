...
【输出格式与字数硬性要求】
⚠️ 这是用户付费购买的深度命理报告。务必写满写透全部16个维度——一个都不能少，不能"略"，不能半途而止。严格控制每个维度字数上限（不要超标，防止token耗尽导致后面章节截断）：前6维每维≤600字，7-9维每维≤700字，10-16维每维≤400字。收尾维度（📅大运、🔮流年、💌命理师私语）绝不允许写到一半戛然而止。

【字数节奏铁律】维度1-6务必控制在500-600字之间，留足token给后续10个维度；宁可精炼不铺张——一句话能说清的不用三句。

你必须严格按照以下16个维度展开，每个维度标题必须用对应emoji开头，维度之间用空行分隔。

维度结构（括号内为目标字数区间，务必遵守）：
1. 📜 四柱八字排盘（400-600字：四柱表格+格局总评+用神忌神）
2. 🔥 十神分析（500-600字：命局中最重要的3-4个十神深度分析+组合效应）
3. 🟤 五行能量分析（400-500字：引用精确数值+旺衰+补泄+脏腑+饮食建议）
4. 💰 财运格局（500-600字：正偏财+财库+发财黄金年份3个+行业5个+投资禁忌）
5. 💕 感情姻缘（500-600字：夫妻宫+正缘特征+遇缘年份+桃花类型+开运建议）
6. 💼 事业格局（500-600字：官杀印+职业路径+升职年月3次+贵人特征）
7. 🏥 健康预警和养生建议（500-600字：脏腑弱项+体检方向+运动作息饮食+食疗方）
8. 📅 全部大运（每步大运150-180字，8步全写，不漏，合计1200-1400字）
9. 🔮 未来10年逐年流年（每年100-130字：三维趋势+吉神+凶神+关键提醒，10年合计1000-1300字）
10. ✨ 神煞分析（300-400字：天乙贵人/桃花/驿马/华盖/文昌，各神煞位置与影响）
11. 🌿 藏干分析（250-300字：逐支藏干列出+透出情况+对命局影响）
12. 👨‍👩‍👧‍👦 父母宫/子女宫/夫妻宫（350-400字：三宫分析）
13. 🎯 开运锦囊（300-350字：幸运颜色自然色名+幸运数字3个+吉方+佩戴物+流年避讳）
14. 📖 古法断语（200-250字：原文+出处+白话全译+对应命主）
15. 🔑 英文名与事业签名（150-200字：3个英文名+五行原因+签名风格）
16. 💌 命理师私语（250-300字：专属心里话+一个发自内心的祝福）
...
【五行生克铁律】生克方向：木生火·火生土·土生金·金生水·水生木。"壬水生乙木"=水生木，壬水是生者（母），乙木是被生者（子）；绝对不能写成"壬水被乙木生"——这是方向写反的严重错误。`,
      `请为我批算八字命盘，生成一份完整的深度命理报告。16个维度全部写完写透，每维度严格控制在字数上限内（详见下方各维度括号），确保16维全部完成不被截断。
...
【输出要求】
请严格按照以下16个维度展开，每个维度用对应的emoji作为标题开头，每个维度都必须基于上述生辰八字展开具体分析，不能以"略"或省略号代替任何内容。⚠️字数铁律：每维度不超过括号内上限——这是确保全部16维度都能写完的关键，务必遵守！

1. 📜 四柱八字排盘（400-600字）
- 照上方【精确排盘结果】誊写四柱八字表格，不得自行推算改动
- 各柱代表的人生领域（年=祖上/早年；月=父母/青年运；日=自身/配偶；时=子女/晚年）
- 格局判断（身强身弱、格局名称、用神忌神）

2. 🔥 十神分析（500-600字）
- 重点分析命局中力量最强的3-4个十神（不必逐一列全部10个）
- 十神组合效应对性格、事业、感情的具体影响

3. 🟤 五行能量分析（400-500字）
- 五行力量：直接引用精确排盘数值，禁止自行编造百分比
- 用神和忌神明确列出（区分调候用神与格局用神/忌神，如火既是调候所需又是格局偏忌，须分层说明避免矛盾）
- 五行脏腑对应及命主体质弱点+饮食建议

4. 💰 财运格局（500-600字）
- 正偏财分析+财库开闭
- 发财黄金年份3个（具体年份+大运流年原因）
- 最适合行业5个+投资禁忌
...
7. 🏥 健康预警和养生建议（500-600字）
- 先天体质弱项（脏腑角度，不点名西医病名）
- 需重点关注的年龄段
- 运动类型+作息建议+饮食禁忌+食疗方四季方案

8. 📅 全部大运（每步150-180字，8步全写，不漏）
- 格式：**[干支]大运（X-X岁，XXXX-XXXX年）**：该步大运与命局生克+对财运/事业/感情综合影响+关键转折年+定性（黄金期/平稳期/挑战期）

9. 🔮 未来10年逐年流年（从${new Date().getFullYear()}年到${new Date().getFullYear()+9}年，每年100-120字）
- 格式：**[年份]年（[干支]年）**：财运趋势 | 感情趋势 | 事业趋势 | 吉神：[具体] | 凶神：[具体] | 关键提醒：[1-2句]

10. ✨ 神煞分析（280-350字）
- 逐一列出：天乙贵人/桃花/驿马/华盖/文昌/孤辰寡宿——有无、位置、具体影响

11. 🌿 藏干分析（230-280字）
- 逐支列出藏干+是否透出+对命局格局影响+有无暗藏用神或忌神

12. 👨‍👩‍👧‍👦 父母宫/子女宫/夫妻宫（320-380字）
- 三宫各自分析：父母缘+子女缘+配偶特质

13. 🎯 开运锦囊（280-350字）
- 幸运颜色（自然色名，解释五行原因，禁止编造色号）+幸运数字3个+吉方+佩戴物+今年流年避讳3-5件

14. 📖 古法断语（200-250字）
- 原文（附出处）+白话全译+对应命主命局分析

15. 🔑 英文名与事业签名（150-200字）
- 3个英文名+五行属性+签名风格建议
...
- 专属心里话（非套话）+发自内心的祝福`
    ...
    if (baziTier === 'free') {
      useMessages = buildReadingPrompt(
        '你是精通八字命理的命理师。为用户生成一份【基础版】命盘概览。必须严格依次输出以下3个章节，一章都不能少，每个章节标题必须以对应的emoji开头（方便客户端解析）:\n📜 四柱八字排盘（约500字：四柱展示+日主强弱+格局总评）\n🌊 五行能量分析（约500字：五行旺衰+喜用忌神+性格特质）\n🌟 今年运势概览（约600字：结合今年干支，写事业/财运/感情/健康整体走势与关键提醒——这一章最重要、绝对不能省略）\n【铁律】必须把上面3章全部写完（尤其🌟今年运势概览不可跳过），三章都写完之后，才能输出一行"---LOCKED---"，然后原样输出以下锁定内容提示（不展开）:\n💰 财运格局 · 完整解读见付费版\n❤️ 感情姻缘 · 完整解读见付费版\n🏆 事业格局 · 完整解读见付费版\n🔑 开运锦囊 · 完整解读见付费版\n语言简体中文、温暖白话、具体有料，让用户感受到真实价值。【合规】今年运势章节最后必须附一句：「本报告由AI辅助生成，仅供参考娱乐，不构成任何决策建议。」' + `【当前时间基准】今年是 ${NOW_Y} 年，所有运势分析以 ${NOW_Y} 年为"今年"。`,
        '请为以下命主生成【基础版】命盘概览：必须依次写满3节——📜四柱排盘、🌊五行能量、🌟今年运势概览（这节最重要、不可省略），每节写实写透，合计约1600字，3节全部写完后再输出---LOCKED---及锁定章节提示):\n出生:' + birthYear + '年' + birthMonth + '月' + birthDay + '日' + (birthHour !== undefined ? birthHour + '时' : '(时辰不详)') + '\n性别:' + (gender === 'male' ? '男' : '女')
      );
    ...
    var freeMaxTokens = baziTier === 'free' ? 4200 : (baziTier === 'standard' ? 6000 : 16384);
...
【真实性铁律·违反即废稿】命主只提供了出生时间与性别。①绝不把假设写成"发生过的事实"——禁止出现"2024年你换了办公室""上个月你失眠""过去三年你接手过某项目"这类杜撰的既往经历或具体人物剧情；要谈倾向请用"你很可能/你这类人往往/你倾向于"。②绝不编造任何量化数字——百分比、评分、"XX%的人"、临床统计（"8.7分""前15%""绩效提升40%"），以及"相当于正常值1/4强度""比同龄人多1-2年积累""过去三年有47天""快3倍以上""0.3秒决策""沉默三秒""连续三天失眠""改到第7版""高于七成""近十年最充盈"这类编造的倍数/秒数/天数/版本/成数/"最X"比较，全部禁止；除非数字逐字来自事实卡（如五行小数、大运起止年份），否则改定性词（"更强/偏弱/明显/为数不多"）。③不写荒诞开运仪式（如"喝蜂蜜水补癸水""某号做小额理财"）。④全文一个百分号(%)都不许出现；五行强弱一律用"偏旺/中和/偏弱/较重/较轻"等档位词，绝不写成百分比或占比数字（如"水40%""金35%"）。⑤绝不虚构任何研究、论文、数据来源或权威出处（如"数据来自《占星与神经科学》研究""某机构统计显示"），命理解读不得伪装成科研结论。
...
【五行生克铁律】木生火·火生土·土生金·金生水·水生木；"壬水生乙木"=水生木，绝不能写反方向。${FMT_LAW_ZH}`;
    ...
    let userPrompt;
    if (full) {
      userPrompt = `${info}\n请出具一份【${cfg.title}】专项深度解读，只写${cfg.key}这一个领域，1200-1600字，写满写透。必须覆盖：${cfg.focus}。用朋友聊天的语气、重点加粗，靠**具体场景与画面感**增强说服力（可引用事实卡里的真实干支/大运起止年份），但严禁编造百分比、评分、倍数、天数、秒数、"第X版"等任何统计数字，也严禁把假设写成发生过的往事。结尾一句温暖的${cfg.key}寄语。`;
    } else {
...
function _dropFakeStats(t) {
  return String(t || '')
    ...
    .replace(/(降本|降低|提升|提高|增长|增加|减少|下降|节省|优化|领先|超出|高出|多出)\s*\d+\s*%/g, '$1不少')
    .replace(/[（(]\s*\d+\s*%\s*[)）]/g, '')
    .replace(/\d+\s*%/g, '较高')
    .replace(/相当于正常值\s*\d+\s*\/\s*\d+\s*强度/g, '明显偏弱')
    .replace(/比同龄人多\s*\d+(-\d+)?\s*年/g, '比同龄人更早起步')
    // ── 0820夜复审补漏：拦住漏过%正则的一大类伪造精确数字（数字+中文数字）──
    .replace(/[\d.]+\s*倍(以上|多)?/g, '数倍')
    .replace(/[一二两三四五六七八九十]+\s*倍(以上|多)?/g, '数倍')
    .replace(/[\d.]+\s*秒(钟)?/g, '片刻')
    .replace(/[一二两三四五六七八九十]+\s*秒(钟)?/g, '片刻')
    .replace(/连续\s*[\d一二两三四五六七八九十]+\s*天/g, '连续多日')
    .replace(/(过去|近|后续|接下来|随后)\s*[\d一二两三四五六七八九十]+\s*(天|周|个月)/g, '$1一段时间')
    .replace(/第\s*[\d一二两三四五六七八九十]+\s*版/g, '反复多轮')
    .replace(/[\d.]+\s*pt\b/gi, '细微')
    .replace(/(高于|超过|多出|领先|低于)\s*[两二三四五六七八九]成(以上|多)?/g, '$1不少')
    .replace(/[\d.]+\s*分\s*[（(]\s*满分\s*[\d.]+\s*[)）]/g, '很高的评价')
    .replace(/近\s*(十|百|几十|二十|三十)\s*年\s*来?最/g, '近年少见地');
}
...
const SESSION_CFG = {
  ziwei: {
    ...
    topics: {
      overview: { free: true, title: '命盘总览', focus: '命宫主星与格局、命主性格天赋与软肋(用你这类命格常见的职场/关系倾向让TA对号入座·只描述倾向不虚构任何具体事件或年份)、命盘最突出的1-2个亮点与要留意处、今年大限一句话概览', lockTitles: [] },
...
❤️ 感情线（心线）— 你的情感模式与爱情风格，在关系里的真实表现（约450字）。这一章结尾必须留一个强烈且【具体】的悬念钩子——${features ? '用你自己的话，从这双手上真实可见的某条纹/某个丘，具体点出一个关于TA感情或人生的关键发现（落到具体特征、制造好奇心' : '用你自己的话，基于TA的手型与整体气质，点出一个关于TA感情或人生的关键倾向（因未上传照片，只作整体倾向判断——绝对不可虚构或点名任何具体掌纹、掌丘、线的走向，制造好奇心'}，绝不能写"你的手藏着秘密"这种空话，也不能照抄本指令措辞），但把"这到底意味着什么、该怎么应对"留到完整版揭晓，让读者读到这里非解锁不可。
...
【收尾合规】报告最后附一行："本报告由AI辅助生成，仅供参考娱乐，不构成医学、法律、投资或人生重大决策建议。"${FMT_LAW_ZH}${langSuffix(zwLang)}`;
...
【当前时间基准】今年是 ${NOW_Y} 年。所有大限/流年分析以 ${NOW_Y} 年为"当前/今年"，禁止写2024/2025年为今年。`;
    ...
    const userMsg = ziweiAccess
      ? `出生：${birthYear}年${birthMonth}月${birthDay}日${birthHour}时\n性别：${gender === 'male' ? '男' : '女'}\n用户关注：${zwQuestion || '请给我全面的紫微斗数命盘解读'}\n\n请按以下维度逐一深度展开。每个维度用【对应 emoji + 标题】作为该章大标题、独占一行，标题里绝对不要加"1. 2. 3."数字编号（数字编号会被系统误切成空白卡）。章内的小标题请用 **加粗文字**，不要用 emoji/数字起小标题：\n\n🌟 命盘格局总览（不少于600字）\n☀️ 命宫主星深度解读（不少于800字）\n💰 财帛宫（财星力量、聚财方式、最佳求财年份3个，不少于500字）\n💕 夫妻宫（正缘星位、配偶特征、遇缘年份、感情模式，不少于500字）\n👨‍👩‍👧‍👦 子女宫（不少于350字）\n🏠 田宅宫（不少于350字）\n💼 官禄宫（事业星、职业路径、升职最佳年份，不少于500字）\n👥 奴仆宫（不少于350字）\n🚀 迁移宫（不少于350字）\n🏥 疾厄宫（养生方向，中医脏腑角度，不点病名，不少于450字）\n🌈 福德宫（不少于350字）\n👪 父母宫（不少于350字）\n🌀 四化飞星（化禄/化权/化科/化忌各自落宫及影响，不少于550字）\n📅 当前大限深批（起止年份、主题、关键年份，不少于550字）\n🔮 未来10年逐年流年（每年财/情/事评分+一句主题，不少于900字）\n🎯 开运锦囊（幸运色、吉方、推荐佩戴物、流年避讳，不少于450字）\n💌 命理师私语（只对此命盘说的心里话，不少于350字）`
      : `出生：${birthYear}年${birthMonth}月${birthDay}日${birthHour}时\n性别：${gender === 'male' ? '男' : '女'}\n\n这是【免费预览版】，只输出以下3节（合计约700字，内容真实有用、说人话让TA一读就"这说的就是我"）。每节标题用 emoji 独占一行，不要数字编号：\n\n🌟 命盘格局总览（2-3段，200字）\n☀️ 命宫主星（2段，200字）\n🌙 今年大限流年（1段，200字）——这一段结尾必须留一个强烈且【具体】的悬念钩子：明确点出今年命盘上有一个关键转折/机会/坎（要具体到与TA命盘相关，禁写"会有转机"这种空话），但把"具体是几月、由哪个宫在推动、该怎么抓/怎么避"全部留到完整版，让TA读到这里心里一紧、非解锁不可。禁止照抄这段措辞，要结合上文命盘自然写出。\n\n完成后输出：---LOCKED---\n💰 财帛宫深度解读 · 完整版解锁\n💕 夫妻宫 · 完整版解锁\n💼 官禄宫 · 完整版解锁\n🌀 四化飞星 · 完整版解锁\n🔮 未来10年流年 · 完整版解锁\n\n最后一行：想看完整紫微斗数报告？解锁后可看到 17 个维度全部深度批算，含精准婚期、事业贵人年份与开运锦囊。`;
    ...
    const streamBody = await deepseekStream([{ role: 'system', content: systemPrompt }, { role: 'user', content: userMsg }], { maxTokens: ziweiAccess ? 16384 : 3000, timeout: 300000, priority: 'deepseek' });
    const reader = streamBody.getReader();
    const decoder = new TextDecoder('utf-8');
    let buf = '';
    while (true) {
